...
const (
	...
	StmtUpdateOneQr        = `UPDATE mysql_query_rules SET active=%d,username=%s,schemaname=%s,client_addr=%s,digest=%s,match_digest=%s,match_pattern=%s,replace_pattern=%s,destination_hostgroup=%d,cache_ttl=%d,error_msg=%s,apply=%d WHERE rule_id=%d`
	StmtUpdateOneQrNoCache = `UPDATE mysql_query_rules SET active=%d,username=%s,schemaname=%s,client_addr=%s,digest=%s,match_digest=%s,match_pattern=%s,replace_pattern=%s,destination_hostgroup=%d,error_msg=%s,apply=%d WHERE rule_id=%d`
)
...
func (qr *QueryRules) UpdateOneQrInfo(db *sql.DB) int {
	...
	if qr.Cache_ttl == 0 {
		...
		st = fmt.Sprintf(StmtUpdateOneQrNoCache,
			qr.Active,
			qr.Username,
			qr.Schemaname,
			qr.Client_addr,
